...
train_data_positive_upsampled = train_data_positive.loc[
    train_data_positive.index.repeat(10)
]

train_data_upsampled = pd.concat(
    [train_data_positive_upsampled, train_data_negative],
    ignore_index=True
)

train_data_upsampled = train_data_upsampled.sample(
    frac=1, random_state=1
).reset_index(drop=True)

train_train_data = train_data_upsampled
